...
.fb-bar .more{flex:none;background:#0A6A2F;color:#fff;text-decoration:none;font-size:.8rem;font-weight:600;padding:7px 11px;border-radius:999px;white-space:nowrap}
.loading{position:fixed;inset:50px 0 0 0;display:flex;align-items:center;justify-content:center;color:#fff;opacity:.7}
.nav button.zoom{width:auto;padding:0 16px;font-size:.9rem;font-weight:700}
.zoomov{position:fixed;inset:0;z-index:50;background:#0b0d09;display:none;overflow:auto;-webkit-overflow-scrolling:touch;touch-action:pinch-zoom}
.zoomov.on{display:block}
.zoomov img{display:block;width:100%;height:auto}
.zoomov .zx{position:fixed;top:10px;right:12px;z-index:52;background:rgba(255,255,255,.95);color:#12140f;border:0;border-radius:999px;width:44px;height:44px;font-size:1.5rem;font-weight:700;box-shadow:0 2px 10px rgba(0,0,0,.4)}
.zoomtip{position:fixed;top:16px;left:12px;z-index:52;color:#fff;font-size:.8rem;opacity:.85;background:rgba(0,0,0,.4);padding:5px 10px;border-radius:999px}
"""
...
<body>
<div class="fb-bar"><a class="back" href="/kit/" aria-label="Quay lại">←</a><div class="tt">{e(name)}</div><div class="ct" id="ct">1 / {len(pgs)}</div><a class="more" href="{detail}">Chi tiết →</a></div>
...
<div class="nav"><button id="prev" aria-label="Trang trước">‹</button><button id="zoom" class="zoom" aria-label="Phóng to đọc">🔍 Phóng to</button><button id="next" aria-label="Trang sau">›</button></div>
<div class="zoomov" id="zoomov"><div class="zoomtip">Chụm 2 ngón để phóng to · vuốt để xem · bấm ✕ để đóng</div><button class="zx" id="zx" aria-label="Đóng">×</button><img id="zimg" alt="Trang phóng to"></div>
...
const zoomov=document.getElementById('zoomov'), zimg=document.getElementById('zimg'),
  zx=document.getElementById('zx'), zoomBtn=document.getElementById('zoom');
zoomBtn.onclick=()=>{{const i=PF?PF.getCurrentPageIndex():0; zimg.src=IMGS[i]||IMGS[0]; zoomov.classList.add('on');}};
zx.onclick=()=>zoomov.classList.remove('on');
